...
extractor = Model(inputs=model.inputs, outputs=model.get_layer('max_pooling1d_35').output)
features_train = extractor(train_sequence)

cov_desc_train = []

for i in tqdm(features_train):
    cmat = tfp.stats.covariance(i, i)
    cov_desc_train.append(cmat.numpy().flatten())

log_desc = []
for i in tqdm(cov_desc_train):
    log_desc.append(np.log(i))

labels = []
for i in tqdm(range(len(train_sequence))):
    data, label = train_sequence[i]
...
clf = MultiOutputClassifier(SVC()).fit(log_desc, labels)

features_val = extractor(validation_sequence)

cov_desc_val = []

for i in tqdm(features_val):
    cmat = tfp.stats.covariance(i, i)
    cov_desc_val.append(cmat.numpy().flatten())

log_desc_val = []
for i in tqdm(cov_desc_val):
    log_desc.append(np.log(i))
...
print(clf.score(log_desc_val, labels_val))
...
#     if(np.isnan(np.sum(val_outputs[-1]))):
#         print(val_outputs[-1])
#         count += 1
# print(count)
